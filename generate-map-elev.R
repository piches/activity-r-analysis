...
ggmap(gm, base_layer = gbase) + geom_path(data = filtered_data, aes(x=Lng, y=Lat, group=GroupID, color=Alt), size = 1.5, lineend = "round") + 
  labs(x="", y="", color="Elevation", title = "All activities", subtitle = paste(dateFrom, "to", dateTo)) +   scale_color_gradientn(colours = rainbow(7), breaks = seq(25, 200, by = 25))
